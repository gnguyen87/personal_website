---
title: "Are bigger ferris wheels with larger capacity more likely to be climate controlled?"
author: Na Nguyen
date: "September 5, 2024"
website: 
  navbar:
      background: dark
      search: true
image: thumbnail.png
categories: 
  - R
  - Interactive Viz
  - Education

about: 
  template: marquee
  image: thumbnail.png
  links: 
    - icon: link-45deg
      text: Website
      href: https://opbfa1-anh-nguyen.shinyapps.io/UK_EduAttainment/
    - icon: github
      text: Github
      href: https://github.com/gnguyen87/uk_edu
---

# Tidy Tuesday, [2022-08-09](https://github.com/rfordatascience/tidytuesday/tree/master/data/2022/2022-08-09)

Source: The [dataset](https://github.com/rfordatascience/tidytuesday/blob/master/data/2022/2022-08-09/wheels.csv)

# Visualization

To accommodate for large crowds at a state fair, we should expect its ferris wheel to be climate controlled. However, there seems to be no discernible difference between those that are climate-controlled and not across the world, irregardless of its capacity and size. It is interesting, though, that all observable "un-climate-controlled" ferris wheels (5) reside in the U.S. (4) and the U.K (1)

# Data Preparation

Here are all the libraries I worked with.

```{r, message=FALSE, warning =FALSE}
#| code-fold: true
library(rnaturalearth)
library(rnaturalearthdata)
library(ggplot2)
library(plotly)
library(tidyverse)
library(gcookbook)
library(scales)
library(cowplot)
library(maps)

```

Loading the Data

```{r,  message=FALSE, warning =FALSE, results='hide', eval = FALSE}
#| code-fold: true
wheels <- read.csv("wheels.csv")
geo<-read.csv("https://raw.githubusercontent.com/plotly/datasets/master/2014_world_gdp_with_codes.csv")
```

Cleaning Data

```{r,  message=FALSE, warning =FALSE,  eval = FALSE  }

df <- wheels %>% 
  mutate(country = case_when(
    country == 'UK' ~ 'United Kingdom',
    country == 'USA' ~ 'United States',
    country == 'Phillippines' ~ 'Philippines',
    country == 'S Korea' ~ 'Korea, South',
    country == 'Tailand' ~ 'Thailand',
    country == 'UAE' ~ 'United Arab Emirates',
    country == 'Dubai' ~ 'United Arab Emirates',
    .default = country
  ),
      climate_controlled = case_when(
    climate_controlled == 'yes' ~ 'Yes',
    .default = climate_controlled
    )) %>% 
  drop_na(climate_controlled) %>% 
  inner_join(geo, by = c('country' = 'COUNTRY')) 
```

# Plot

```{r, eval= FALSE}
# Define UI for application that draws a histogram
plot_ly(df, x = ~seating_capacity, y = ~hourly_capacity, text = ~paste("Name:", name, "<br>","Country:", country,"<br>","Diameter:", diameter, "<br>","Height:", height), type = 'scatter', mode = 'markers', size = ~diameter, color = ~climate_controlled, colors = c('#8B0000','#008000'),
        marker = list(opacity = 0.5, sizemode = 'diameter')) %>% 
  layout(title = "Climate Controlled Ferris Wheels Around The World (by capacity and size)",
         xaxis = list(
           zerolinecolor = '#ffff',
           zerolinewidth = 2,
           gridcolor = 'ffff'),
         yaxis = list(
           zerolinecolor = '#ffff',
           zerolinewidth = 2,
           gridcolor = 'ffff') )

```

# Credit

This is the incredible [source](https://r-charts.com/distribution/ggridges/) from R Charts that helped me with my quantile ridgeline plot
